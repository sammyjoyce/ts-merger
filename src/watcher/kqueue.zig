// Do not fundamentally change this approach
...
pub const KqueueWatcher = struct {
    ...
    fn watcherThread(self: *KqueueWatcher) void {
        ...
        while (self.running.load(.acquire)) {
            ...
            for (events[0..@intCast(n)]) |evt| {
                const fd = @as(i32, @intCast(evt.ident));
                var it = self.watched_paths.iterator();
                while (it.next()) |entry| {
                    if (entry.value_ptr.*.fd == fd) {
                        // For directories, we need to handle new files
                        if (entry.value_ptr.*.is_dir and (evt.fflags & c.NOTE_WRITE) != 0) {
                            self.scanDirectory(entry.value_ptr.*.path) catch continue;
                            continue;
                        }

                        const kind = if ((evt.fflags & c.NOTE_DELETE) != 0)
                            mod.WatchEvent.EventKind.delete
                        else if ((evt.fflags & c.NOTE_WRITE) != 0)
                            mod.WatchEvent.EventKind.modify
                        else if ((evt.fflags & c.NOTE_RENAME) != 0)
